...
The methodology is fully described in \cite{ballin:2013}, and a complete illustration of the package, together with 
a comparison with the {\tt stratification} package, is in \cite{barcarol:2013}.
Also a complete application in a case of network data is reported in \cite{ballin2016optimization}.
The implementation of the genetic algorithm is based on a modification of the functions in the {\tt genalg} package (see \cite{willighagen:2005}).
In particular, the crossover operator ha been modified on the basis of the indications given by \cite{oluing:2017}.
...
Function {\tt buildFrameDF} permits to organize data in a suitable mode for next processing:
...
A suitable way to do so, is to apply a k-means clustering method (see \cite{hartigan:1979}) by using the function {\tt var.bin}:
<<frame4>>=
library(SamplingStrata)
...
\item	{\tt cens}: the (optional) dataframe containing the 'take-all' strata, those strata whose units must be selected in whatever sample. 
It has same structure than {\tt strata} dataframe 
\item	{\tt strcens}: flag (TRUE/FALSE) to indicate if 'take-all' strata do exist or not. Default is FALSE 
\item	{\tt initialStrata}: This is the initial limit on the number of strata for each solution. Default is the number of atomic strata. In cases of more than 
a domain, it has to be given as a vector, where each element indicates the number of initial strata in each domain.
If the parameter {\tt addStrataFactor} is equal to zero, then {\tt initialStrata} is equivalent to the maximum number of strata to be obtained
...
Default is FALSE.
\item	{\tt writeFiles} : indicates if at the end of the processing the resulting strata will be outputted in a delimited file.
Default is TRUE.
\end{enumerate}
...
@
Note how the {\tt initialStrata} parameter is set: by giving the number of the strata in each domain. Another possibility is to set a
pre-determined value for each domain, for instance equal in each domain, as {\tt c(5,5,5,5,5,5,5,5)}.
...
\section[Adjustment of the final sampling size]
...
\begin{enumerate}
\item	the resulting sample size is acceptable;
\item	the resulting sample size is to high, it is not affordable with respect to the available budget;
\item the resulting sample size is too low, the available budget permits to increase the number of units.
\end{enumerate}
...
\section[Handling 'take-all' strata in the optimization step]
...
In order to correctly execute the optimizaton and further steps, it is necessary to perform a pre-processing of the overall input.
...
sum(strata$N)
@
Now we have all required inputs to run 'optimizeStrata' in presence of 'take-all' stata:
<<frame25>>=
solution <- optimizeStrata(
	errors = swisserrors, 
	strata = strata, 
