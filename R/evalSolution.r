evalSolution <- function (frame, outstrata, nsampl=100) 
{
    ...
    for (i in 1:numY) {
		stmt <- paste("param[i,] <- aggregate(frame$Y",i,",by=list(frame$DOMAINVALUE),FUN=sum)[,2]",sep="")
...
#			samp <- read.csv("sample.csv")
	...
	cv <- as.data.frame(cv)
    for (i in 1:numY) {
        stmt <- paste("colnames(cv)[",i,"] <- c('CV",i,"')", sep = "")
        eval(parse(text = stmt))
    }	
	...
	for (k in (1:numY)) {
		for (j in (1:numdom)) {
			...
			stmt <- paste("cv1$val[(k-1)*numdom + j] <- cv$CV",k,"[j]",sep="")
			eval(parse(text=stmt))
		}
	}
...
#	cv
	...
	for (i in (1:numY)) {
		stmt <- paste("diff$diff",i," <- rep(0,numdom*nsampl)",sep="")
		eval(parse(text=stmt))
	}
	...
	for (i in (1:numdom)) {
		for (j in (1:nsampl)) {
			...
			for (k in (1:numY)) {
				stmt <- paste("diff$diff",k,"[(i-1)*nsampl+j] <- differ[i,j,k]",sep="")
				eval(parse(text=stmt))
			}
		}
	}
