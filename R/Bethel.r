...
#                  - Survey Methodology                       
# Author: Daniela Pagliuca <pagliuca@istat.it> 
#         with contributions of M. Teresa Buglielli <bugliell@istat.it> 
#         and Giulio Barcaroli <barcarol@istat.it>          
#
###############################################################
`bethel` <-
function (
...
#   
         ordina_variabili=function(dati,prefisso, n_var)
         {
           if (!is.data.frame(dati) ) 
               stop()
           as.matrix(dati[,paste(prefisso,1:n_var,sep="" ),drop=FALSE])
         }        
#--------------------------------------------------------------
...
#-------------------------------------------------
        ...
        crea_disj=function(data,vars)
        {
           ...
           sapply( vars, function(vari){ 
                ...
                out<<- cbind(out, outer(col, levels(col), function(y,x) ifelse(y==x,1,0))) 
                })
           out
        }
        ...
        for (k in 1:ndom) 
         {
             ...
             { 
                ...
                cvDom2 <- c(cvDom2, rep(levels(as.factor(dom[,k]))[k1], length(cvx)))
             }    
         }
...
#---------------------
     ...
     stampa_confronto <- function (n, N, strato) {
          nomi <- c("STRATUM", "POPULATION","BETHEL", "PROPORTIONAL", "EQUAL")
          df <- NULL
          df <- cbind(df, as.character(strato), N, n,  ceiling(sum(n)*N/sum(N)), ceiling(sum(n)/length(n)))
          tot <- apply (matrix(as.numeric(df[,2:5]),ncol=4),2, sum)                 
...
#---------------------
   stampa_cv <- function ()    {
         ...
         for (i in 1:ndom) 
           domcard <- c(domcard, nlevels(dom[,i]))
        
         tit_cv = c("TYPE", "DOMAIN/VAR.",  "PLANNED CV ",  "ACTUAL CV",  "SENSITIVITY 10%")
         outcv <- cbind ( as.vector(cvDom), 
                               paste(as.vector(cvDom2),"/V",c(1:ncol(med) ), sep=""), 
                               as.vector(cv), as.vector(CVfin), as.vector(ceiling(sens)))
...
checkData <-
function( strata = NULL,errors = NULL) {
